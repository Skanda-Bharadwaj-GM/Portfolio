...
export default function SocialLinks({ variant = 'footer' }: { variant?: 'footer' | 'compact' }) {
  const isCompact = variant === 'compact'
  return (
    <div className={isCompact ? 'flex items-center gap-4' : 'flex items-center justify-center gap-8 py-6'}>
      {links.map(({ name, href, icon: Icon }) => (
        <a
          ...
          className={`text-zinc-400 hover:text-accent transition-colors duration-200 ${isCompact ? 'w-8 h-8' : 'w-10 h-10'}`}
          aria-label={name}
        >
          <Icon className={isCompact ? 'w-6 h-6' : 'w-8 h-8'} />
        </a>
      ))}
    </div>
  )
